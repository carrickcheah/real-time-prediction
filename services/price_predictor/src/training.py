...
def train_model(
    ...
    dataset_hash = hash_dataframe(ohlc_data)                                # hash the dataset
    experiment.log_parameter("ohlc_data_hash", dataset_hash)

    # split the data into training and testing
    logger.debug(f"Splitting the data into training and testing")           # log the start of the process
    test_size = int(len(ohlc_data) * perc_test_data)                        # 30% of the data is used for testing
    train_df = ohlc_data[:-test_size]                                       # the rest is used for training
    test_df = ohlc_data[-test_size:]                                        # the last 30% of the data is used for testing
    logger.debug(f"Training data: {len(train_df)} rows")                    
    logger.debug(f"Testing data: {len(test_df)} rows")                      
    experiment.log_parameter("n_train_rows_before_dropna", len(train_df))   # log the number of rows before dropping NaN values
    experiment.log_parameter("n_test_rows_before_dropna", len(test_df))     # log the number of rows before dropping NaN values

    # add a column with the target price we want our model to predict
    # for both training data and testing data
    train_df['target_price'] = train_df['close'].shift(-forecast_steps)     # shift the close price by forecast_steps
    test_df['target_price'] = test_df['close'].shift(-forecast_steps)       # shift the close price by forecast_steps
    ...
    X_train = keep_only_numeric_columns(X_train)                            # keep only the numeric columns
    X_test = keep_only_numeric_columns(X_test)                              # keep only the numeric columns
